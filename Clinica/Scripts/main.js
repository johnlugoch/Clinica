...
function ListaPaciente() {
    $.ajax({
        type: "GET",
        contentType: "application/json; charset=utf-8",        
        url: "/Paciente/VerPaciente",
        dataType:'json',
        success: function (data) {
            tableData = data;
            
            var rowH = "<tr class='azul'><th>DOCUMENTO</th><th>APELLIDO 1</th><th>APELLIDO 2</th><th>NOMBRE</th></tr>";
            $("#tbl").append(rowH);
            jQuery.each(tableData, function (i, tableData) {
                var rowHTML = "<tr><td>" + tableData.id + "</td><td>" + tableData.papellido + "</td><td>" + tableData.sapellido + "</td><td>"
				+ tableData.pnombre + "</td><td><button id='editar' type='button' onclick='miFuncion2(" + tableData.id + ")' class='btn-success' ><span class='glyphicon glyphicon-edit' style='color:#fff;font-size: 15px;'></span> </button></td>" +
